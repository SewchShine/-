        ...
        <table class="result-table">
          ...
          <tbody>
            <tr v-for="(order, index) in orders" :key="order.orderId">
              ...
              <td>
                <button class="btn primary" @click="openDetail(index)">查看详情</button>
                <button class="btn success" @click="handleApprove(index)">通过</button>
                <button class="btn danger" @click="handleRevoke(index)">撤销</button>
              </td>
            </tr>
          </tbody>
        </table>
...
const handleApprove = (index) => {
  ...
  stats.value.normal.reviewed += 1
  stats.value.normal.pending -= 1
  alert(`订单 ${order.orderId} 审核通过`)
}
